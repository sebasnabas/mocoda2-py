...
def main():
    ...
    parser.add_argument('-m', '--number-of-messages', type=int, default=10,
                        help='Max number of messages per chat participant')
    parser.add_argument('-p', '--number-of-participants', type=int, default=20,
                        help='Max total number of chat participants')
    ...
    max_participants = min(len(participants), args.number_of_participants)

    filtered_participants = filter_participants(participants, age_groups,
                                                max_participants, args.number_of_messages)

    filename = f"mocoda2_aged_{args.age_groups}_participants_{len(filtered_participants)}_messages_{args.number_of_messages}.csv"
    ...
    print(f"Wrote {len(filtered_participants)} participants with max {args.number_of_messages} messages to {filename}")
